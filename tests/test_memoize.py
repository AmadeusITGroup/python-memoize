...
from memoize import Memoizer, DefaultCacheObject, _get_argspec, function_namespace
from mock import MagicMock, patch

class MemoizeTestCase(unittest.TestCase):
    def setUp(self):
        self.memoizer = Memoizer(fakeredis.FakeStrictRedis())
    ...
    def test_03_delete_many(self):
        ...
        assert self.memoizer.get('hi') is None
        assert self.memoizer.get('bye') is None
    ...
    def test_07b_delete_memoized_verhash(self):
        ...
        assert (
            self.memoizer.get(version_key) is None
        )
    ...
    def test_17_memoize_none_value(self):
        ...
        assert (
            self.memoizer.get(cache_key) is None)
        result = foo()
    ...
    def test_21_memoize_get_backend_error(self, memoizer_get):
        ...
        memoizer = Memoizer(fakeredis.FakeStrictRedis())
